...
`4b74fda`、`f2e7963`、`04ac747`、`b84fcd1`、`2450ad5`、`2ad0b60`、`e829556`。这些提交均为独立
阶段提交，没有推送、合并或部署。
...
用户随后授权直接访问现有生产部署 `https://chisha.leftzzzz.top` 做黑盒验收，但没有授权
推送、合并、部署或 D1/DO 管理操作。验收只使用普通浏览器/API 用户路径；这些路径按产品
契约创建和读取匿名会话，没有运行 Wrangler、D1 或 Durable Object 管理命令。

现网基线结果：

- 公网 HTTP 对同 path/query 返回 308，并保留到 HTTPS；HTTPS 首页返回 200，地图运行时
  配置为已配置；
- 1280x720 浏览器在上海人民广场测试坐标搜索“想吃火锅，人多的地方”，Agent 返回
  HTTP 200 与 `text/event-stream`。流中包含 4 个 heartbeat，约 72 秒后完成；trace
  显示高德召回 39 家、12 家可进主推荐，最终发布 8 家主推荐和 0 家候补；
- 8 家主推荐进入转盘，转盘可以抽取结果并显示餐厅详情；
- 同一匿名会话继续输入“附近有什么好吃的”时进入追问，再回答“想换成日料”后，服务端
  明确记录“替换主目标为日料”，返回 8 家日料；两个请求使用同一 session；
- 同 owner Cookie 读取该 session 返回 200，另一个无 Cookie 浏览器上下文读取同一 id
  返回 404；
- 390x844 结果页 `scrollWidth` 等于 390，没有横向溢出；页面没有 console error 或
  page error。地图重绘期间只有被浏览器取消的瓦片请求，没有其他失败请求。

这不是本次目标版本的最终线上验收。远端 `main` 当前为 `c78c67b`，现网仍不是包含
R05、R08、R09、R06、R07 和最终对抗修复的目标版本。当前本地目标实现还包含
`e829556`；该提交分离了目标变更理由与用户未满足约束，并让追问动作摘要只使用用户可见的问题文本。
黑盒结果
也直接暴露了 R05 差异：现网 trace 已判定 12 家可进主推荐，转盘上限取 8 家后候补仍为
0；本地 R05 会把超过主推荐上限的合格结果保留到候补分区。因此现网只能证明旧版本的真实
模型、高德、SSE、转盘、多轮和 owner 隔离基线健康，不能证明当前 worktree 的实现已通过
线上验收。发现版本不一致后，没有继续制造额外供应商请求来测试取消或故障注入。

因此当前可以确认：

- R05、R08、R09、R06、R07 的已批准本地实现、阶段提交、全量回归、离线评测、构建、
  Cloudflare dry-run、浏览器失败关闭与恢复路径均已完成；
- 现网旧版本已经实证真实模型、高德、SSE 心跳、主推荐、转盘、多轮目标替换和 owner
  隔离，但 R05 候补分区与当前本地实现不一致；
- 当前目标版本的真实 usage/延迟、主推荐/候补分区、取消、重连和部分失败仍未实证；
- 没有执行生产 D1/DO 管理操作、推送、合并或部署。

### 后续对抗性修复与线上取消实验（2026-09-18）

`e829556 fix: 分离目标变更理由与用户提示` 修复了两个会影响业务解释的语义问题：

- `GoalPatch.reason` 是“目标版本为什么改变”的内部变更理由，不是用户约束未满足的证据。
  之前把它追加进 `UserGoal.ambiguity`，会被 FinalGuard 汇总成 `unmetConstraints`，让用户
  误以为自己的约束没有满足。现在保留既有 ambiguity，回归覆盖目标更新后的
  `unmetConstraints` 不再出现该内部理由。
- `ask_user` 动作的 `question.reason` 是内部诊断信息，`question.question` 才是用户真正看到
  的问题。动作摘要现在只使用后者，避免内部解释、长度不可控文本或实现细节进入 SSE、会话
  记录和前端动作展示。

本地定向回归、`npm run type-check` 和 `git diff --check` 已通过；该修复没有改变目标追加/
替换代数、FinalGuard 准入或外部 Provider 契约。

随后在旧线上版本做了有界浏览器取消实验：请求进入 `searching` 后关闭整个标签页，约 48 秒
内没有观察到 `model_call.outcome=cancelled`；同一 session 随后立即重试返回 HTTP 200、没有
429，并进入 `question` / `session_paused`。这能支持“本次实验没有暴露租约长期滞留”的有限
结论，但不能证明取消信号已从浏览器传播到 Worker，更不能证明目标版本的取消实现已在线通过。
因此 R09 的取消传播保持开放，不追加重复供应商请求来制造故障注入证据。

真实模型多轮实验还观察到“再加上日料”曾产生 `replace*` 而非 R06 设计预期的 `add*`；最终
目标状态正确，但结构化关系仍有契约缺口。当前不使用启发式强行改写模型结果，而是把它保留为
目标版本 live eval 的开放项。

### 目标版本线上验收与最终对抗复核（2026-09-18）

在用户明确授权后，`f831640 fix: 禁用不足三项时的转盘按钮` 已推送到 `origin/main` 并通过
`npm run deploy:skip-migrations` 部署。Cloudflare 返回 Version ID
`42fdd15c-85e7-4193-b488-73a542da7272`；本次没有执行生产 D1 migration，也没有直接管理生产
D1 或 Durable Object。

目标版本发布前的当前门禁均通过：

- `npm run test:ci`：74 suites / 721 tests；
- `npm run eval`：offline 15/15；
- `npm run build`、`npm run build:cloudflare` 和 `npm run deploy -- --dry-run`；
- 前序阶段已通过 `npm run type-check`、`npm run lint`、`git diff --check`；本次文档更新后
  另行重跑文档检查。

目标版本真实浏览器验收使用 `https://chisha.leftzzzz.top`、Chrome、上海人民广场测试坐标、
真实模型、真实高德和真实 SSE：

- 自动定位和反向地理编码返回 HTTP 200；Agent SSE 返回 HTTP 200 与 `text/event-stream`；
  真实搜索无 console error 或 page error；
- “想吃川渝麻辣火锅，人多的地方”进入真实补充需求追问。补充“优先川渝口味的火锅店，
  预算可以放宽到150元，适合多人聚餐”后，目标版本返回 1 家主推荐和 19 家候补；管理面板
  显示 `转盘上 (1/8)`，开始转动按钮的真实 DOM 状态为 `disabled=true`；
- 从候补池显式添加两家后，面板变为 `转盘上 (3/8)`，按钮变为可用；真实转盘完成旋转并
  选中餐厅，`localStorage` 写入 `chisha_history`，历史页显示 1 条记录。候补添加是用户
  操作，不改变后端 FinalGuard 的主推荐资格；
- 另一条明确预算搜索返回 `转盘上 (8/8)` 和 `候补池 (9)`，证明目标版本的主推荐/候补分区
  已经部署，候补没有被自动补进转盘；
- 390x844 移动视口的 `scrollWidth` 等于 390，没有横向溢出；地图配置和真实地图相关请求
  正常，控制台没有新增错误。

对抗性复核还做了一个有限取消探针：真实浏览器在 SSE 收到 `thinking/status` 后主动 abort，
客户端得到 `AbortError`，没有收到迟到的 `final/done`。该探针没有拿到 session id，因此不能
证明取消后的 runtime snapshot 已持久化、重连不会重复外部调用，或 usage/延迟统计完整；
R09 的恢复、usage 和真实取消 trace 仍保持开放。真实模型的“追加/替换”结构化关系也没有在
本轮重新关闭，不能用最终文本看起来合理来替代 `add*` / `replace*` 契约证据。

因此，目标版本已经通过本地门禁、真实成功搜索、主推荐/候补分区、转盘最少项保护、候补显式
加入、历史写入和移动端布局验收；完整项目仍不能宣称所有 R05-R09 线上验收闭合，剩余边界
是 R06 的真实追加/替换分布，以及 R09 的取消快照、恢复、usage/延迟和部分失败实证。
