...
> **历史实施方案，仅作演进记录。** 本文记录了当前确定性 policy 主控 workflow 的形成
> 过程；新增行为以当前 Spec 和架构决策为准。当前生效决策见
...
### 9.3 三个实现过程中引入又被抓出来的回归

前两个由 golden case 与单测挡住，第三个由复查发现。记下来避免重犯：
...
3. **并发写指标互相覆盖。** 联想词写 turn 容器、并发首批的评估写 context，
   收尾整体替换会丢掉先写的那一批。触发条件很窄（Supervisor 走确定性短路
   导致 turn 容器起初为空），offline eval 也测不到——模型是桩。修复是让
   两个容器合成一个：context 在联想词发起时已经存在，直接当 sink。
   已补 `__tests__/lib/agent/runtimeV3.metrics.test.ts` 守住。

   教训：**并发化改造要连同"谁往哪个容器写"一起过一遍**，共享可变状态
   在串行下是安全的，并行下不是。
