...
3. failed 只用于**真冲突**：品类根本不可能供应该菜品，或命中排除项、停业、距离硬约束。
   注意你拿到的事实字段里**没有菜单**，只有店名、品类、地址等。因此"店名里没写这道菜"
   永远不是 failed 的理由——那只是缺证据，应输出 unverified。
   例：目标「柠檬茶」，冷饮店/奶茶店/茶饮店即使店名是「喜茶」「蜜雪冰城」「CoCo都可」，
   也必须是 unverified（品类完全可能供应），不是 failed；只有像「川菜馆」「五金店」
   这种品类不可能供应柠檬茶的才是 failed。
   拿不准时一律 unverified——把可能合适的店判成 failed，用户就彻底看不到它了。
